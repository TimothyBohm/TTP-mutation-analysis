...
# ── Configuration ────────────────────────────────────────────────────────────
FILE_1  = "../Results/1_step/global_summary_one_step_home_away.csv"         # ← change to your first CSV path
FILE_2  = "../Results/1_step/global_summary_one_step_round_swap.csv"         # ← change to your second CSV path
...
OUTPUT  = "graphs/team_size_feasible_ratio.png"

COLOR_DARK  = "#1B4F8A"   # deep navy  – file 1 (taller, behind)
COLOR_LIGHT = "#1CAD55"   # sky blue   – file 2 (shorter, in front)
EDGE_COLOR  = "#0D2B4E"
# ─────────────────────────────────────────────────────────────────────────────
...
def load(path):
    ...
    df = pd.read_csv(path)
    required = {"team_size", "feasible_ratio"}
    if not required.issubset(df.columns):
        sys.exit(f"[ERROR] {path} must contain columns: {required}")
    # Average feasible_ratio per team_size in case there are multiple rows
    return df.groupby("team_size", as_index=False)["feasible_ratio"].mean()
...
ratio1     = merged["feasible_ratio_1"].values   # always larger (behind)
ratio2     = merged["feasible_ratio_2"].values   # always smaller (in front)
n          = len(team_sizes)
x          = np.arange(n)
...
# Draw file 1 first (taller, behind), then file 2 on top (shorter, in front)
...
# ── Value labels on top of each visible bar ───────────────────────────────────
for i, (r1, r2) in enumerate(zip(ratio1, ratio2)):
    # File 1 (taller) — label above the bar
    ax.text(i, r1 + 0.003, f"{r1:.3f}", ha="center", va="bottom",
            fontsize=9, color=COLOR_DARK, fontweight="bold")
    # File 2 (shorter) — label inside the bar near the top
    ax.text(i, r2 - 0.008, f"{r2:.3f}", ha="center", va="top",
...
ax.set_ylim(0, min(1.0, max(ratio1.max(), ratio2.max()) * 1.18))
ax.tick_params(axis="y", labelsize=9, colors="#2C3E50")
ax.tick_params(axis="x", length=0)

# ── Title ─────────────────────────────────────────────────────────────────────
ax.set_title("Feasible Ratios by Team Size (2 step)",
