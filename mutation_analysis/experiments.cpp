...
void run_random_walk_save_feasible_hits(
    ...
    out << "walk_id,start_schedule_id,team_size,step,hit_id,"
           "noRepeat,maxStreak,doubleRoundRobin,total,schedule_key\n";
        ...
        for (int step = 0; step < walk_length; step++) {
            ...
            if (is_feasible(v)) {
                out << schedule_id << ","
                    ...
                    << v.doubleRoundRobin << ","
                    << v.total << ","
                    << schedule_to_key(current) << "\n";

                hit_id++;
            }
        }

        std::cout << "[Feasible Hits] Finished walk "
                  << (schedule_id + 1) << "/" << limit
                  << ", hits so far: " << hit_id << "\n";
    }
